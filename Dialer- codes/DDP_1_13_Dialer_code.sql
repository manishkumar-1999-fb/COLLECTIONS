CREATE OR REPLACE TASK TABLEAU.CREDIT.run_collections_dialer_audit_list_table
    WAREHOUSE = KEY_METRICS_WH
    SCHEDULE = 'USING CRON 15 16 * * * Asia/Kolkata'
AS
BEGIN

------------------------------------------------------------------------------------------------
--ALL customers currently Delinquent
CREATE OR REPLACE TABLE tableau.credit.collections_dialer_audit_list AS
...
-- ALTER TASK TABLEAU.CREDIT.run_collections_dialer_audit_list_table RESUME;


-- Show tasks like 'run_collections_dialer_audit_list_table'


-- Select * from tableau.credit.collections_dialer_audit_list
-- where dpd_bucket in (1,2) and term_loan_exclusion = 1



-- SELECT * FROM tableau.credit.collections_dialer_audit_list
-- WHERE PARTNER_ATTRIBUTION ILIKE '%AUTOBOOK%'
