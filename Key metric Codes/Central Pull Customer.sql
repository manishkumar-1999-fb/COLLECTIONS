...
WITH CTE_USAGE AS (
    SELECT A.FBBID,
     CASE
               WHEN dayofweek(current_date()) = 3 THEN DATE_TRUNC('WEEK', A.EDATE::date+4)::date+2
               WHEN datediff('day', A.EDATE, DATE_TRUNC('WEEK',current_date()+4)::date-5) < 0 AND datediff('day', A.EDATE, current_date()) <= 0 THEN NULL 
               WHEN datediff('day', A.EDATE, DATE_TRUNC('WEEK',current_date()+4)::date-5) < 0 THEN current_date()-1
               ELSE DATE_TRUNC('WEEK', A.EDATE::date+4)::date+2
           END week_end_date,
    max(case when (b.edate between dateadd('month', -6, a.edate) and a.edate) and b.dpd_days <= 35 then (b.outstanding_principal / nullifzero(b.credit_limit)) else null end) as max_util_l6m,
    avg(case when (b.edate between dateadd('month', -3, a.edate) and a.edate) and b.dpd_days <= 35 then (b.outstanding_principal / nullifzero(b.credit_limit)) else null end) as avg_util_l3m,
    FROM BI.PUBLIC.DAILY_APPROVED_CUSTOMERS_DATA A
    left join bi.public.daily_approved_customers_data as b
    on true
    and a.fbbid = b.fbbid
    and b.edate >= a.edate - 300 
...
, CTE_NFBX_WEEKLY AS (
    -- This CTE assumes your view 'indus.public.alternative_lenders_WAL_SHARE'
    -- already aggregates NFBX_CREDITS by fbbid and its own 'week_END_DATE' column.
    SELECT
        fbbid,
        week_END_DATE, -- This is the column name from your view for the weekly end date
        NFBX_CREDITS   -- This is the sum of credits from your view for that fbbid and week_END_DATE
    FROM indus.public.alternative_lenders_WAL_SHARE
),

CTE_DACD_WITH_WEEKLY_DATA AS (
    SELECT
        base.*, -- Carries all columns from CTE_DACD_BASE (fbbid, week_end_date, and all other original DACD fields)
        COALESCE(fmd_join.originated_amount, 0) AS weekly_originated_amount,
        nfbx_join.fbbid IS NOT NULL AS has_nfbx_this_week, -- True if NFBX record exists for this specific week for this fbbid
        COALESCE(nfbx_join.NFBX_CREDITS, 0) AS weekly_nfbx_credits
    FROM CTE_DACD_BASE base
    LEFT JOIN CTE_FMD fmd_join
        ON base.fbbid = fmd_join.fbbid AND base.week_end_date = fmd_join.week_end_date
    LEFT JOIN CTE_NFBX_WEEKLY nfbx_join
        ON base.fbbid = nfbx_join.fbbid AND base.week_end_date = nfbx_join.week_END_DATE
),

FINAL_DACD_METRICS AS (
    SELECT
        -- Select all original DACD columns explicitly from dwd (which inherited them from CTE_DACD_BASE via base.*)
        dwd.fbbid, dwd.week_end_date, dwd.week_start_date, dwd.sub_product, dwd.credit_limit, dwd.credit_status,
        dwd.credit_status_reason, dwd.credit_status_comment, dwd.credit_sub_status, dwd.credit_limit_reason,
        dwd.credit_limit_decision_mode, dwd.credit_limit_comment, dwd.account_status, dwd.account_status_reason,
        dwd.account_status_comment, dwd.account_status_decision_mode, dwd.dpd_days_corrected, dwd.is_chargeoff,
        dwd.chargeoff_time, dwd.is_locked_dashboard, dwd.dashboard_status_change_time, dwd.dashboard_status_change_reason,
        dwd.is_disabled, dwd.is_active_account, dwd.is_xl, dwd.xl_status, dwd.cip_connected_time, dwd.cip_type,
        dwd.recovery_suggested_state, dwd.recovery_suggested_substate, dwd.recovery_max_suggested_state,
        dwd.delinquency_bucket, dwd.delinquency_level, dwd.last_increase_time, dwd.last_decrease_time,
        dwd.first_draw_amount, dwd.first_draw_time, dwd.last_draw_time, dwd.loans_created, dwd.credited_amount,
        dwd.default_principal, dwd.default_principal_paid, dwd.default_fees_paid, dwd.last_good_debit_date,
        dwd.fee_rate_52, dwd.fundbox_plus_status, dwd.credit_utilization, dwd.fees_due, dwd.outstanding_principal,
        dwd.discount_pending, dwd.AVAILABLE_BALANCE_FI, dwd.is_urp_eligible, dwd.fi_data_update_to_time,
        dwd.PLATFORM_DATA_UPDATE_TO_TIME, dwd.CREDIT_LIMIT_CHANGE,
        -- End of original DACD columns

        -- Rolling sum of NFBX credits for the last 4 weeks
        SUM(dwd.weekly_nfbx_credits) OVER (
            PARTITION BY dwd.fbbid
            ORDER BY dwd.week_end_date ASC
            ROWS BETWEEN 3 PRECEDING AND CURRENT ROW
        ) AS nfbx_credits_last_4_weeks,

        -- Conditional rolling sum of originated amounts for the last 4 weeks
        CASE
            -- Check if there was any NFBX activity for this fbbid in the 4-week window
            WHEN MAX(CASE WHEN dwd.has_nfbx_this_week THEN 1 ELSE 0 END) OVER (
                                PARTITION BY dwd.fbbid
                                ORDER BY dwd.week_end_date ASC
                                ROWS BETWEEN 3 PRECEDING AND CURRENT ROW
                            ) = 1
            -- If yes, calculate the sum of originated amounts in that window
            THEN SUM(dwd.weekly_originated_amount) OVER (
                                PARTITION BY dwd.fbbid
                                ORDER BY dwd.week_end_date ASC
                                ROWS BETWEEN 3 PRECEDING AND CURRENT ROW
                            )
            -- Otherwise, return 0
            ELSE 0
        END AS originated_amount_last_4_weeks
    FROM CTE_DACD_WITH_WEEKLY_DATA dwd
)
...
SELECT 
   ...
   , DACD.originated_amount_last_4_weeks AS fbx_orig
   , (DACD.nfbx_credits_last_4_weeks + DACD.originated_amount_last_4_weeks) AS DENOM_WALLET
   --, CASE WHEN DL.FBBID IS NOT NULL THEN ORI.fbx_orig ELSE 0 END AS fbx_orig
   --, CASE WHEN DL.FBBID IS NOT NULL THEN ORI.fbx_orig + DL.ALT_MONTH_ORIG ELSE 0 END AS DENOM_WALLET
   , in_call.call_count as call_count
   , in_call.call_weekend_date as call_weekend_date
   ...
   , FD.tenure_status
   , FD.customer_annual_revenue
 --  , fd.customer_annual_revenue_group
/*   , CASE WHEN FD.customer_annual_revenue>0 and FD.customer_annual_revenue<=150000 THEN '$0 - $150K'
          WHEN FD.customer_annual_revenue>150000 and FD.customer_annual_revenue<=500000 THEN '$150K - $500K'
          WHEN FD.customer_annual_revenue>500000 and FD.customer_annual_revenue<=1000000 THEN '$500K - $1M'
          WHEN FD.customer_annual_revenue>1000000 THEN '> $1M'
          ELSE 'Other/No Data' 
        END AS customer_annual_revenue_group*/
    , CASE WHEN FD.customer_annual_revenue>0 and FD.customer_annual_revenue<=500000 THEN '$0 - $500K'
           WHEN FD.customer_annual_revenue>500000 and FD.customer_annual_revenue<=1500000 THEN '$500K - $1.5M'
           WHEN FD.customer_annual_revenue>1500000 THEN '> $1.5M'
--           WHEN customer_annual_revenue>1000000 THEN '> $1M'
           ELSE 'Other/No Data'
        END AS customer_annual_revenue_group
   , CASE WHEN dl.DEBT_LOAD>=0 and dl.DEBT_LOAD<=0.25 THEN '<=25%'
          WHEN dl.DEBT_LOAD>0.25 and dl.DEBT_LOAD<=0.5 THEN '25%-50%'
          WHEN dl.DEBT_LOAD>0.5 and dl.DEBT_LOAD<=0.75 THEN '50%-75%'
          WHEN dl.DEBT_LOAD>0.75 THEN '>75%'
          ELSE 'Other/No Data' 
        END AS DEBT_LOAD_group
   /*, CASE
          WHEN F.customer_annual_revenue_ob>0 and F.customer_annual_revenue_ob<=150000 THEN '$0 - $150K'
          WHEN F.customer_annual_revenue_ob>150000 and F.customer_annual_revenue_ob<=500000 THEN '$150K - $500K'
          WHEN F.customer_annual_revenue_ob>500000 and F.customer_annual_revenue_ob<=1000000 THEN '$500K - $1M'
          WHEN F.customer_annual_revenue_ob>1000000 THEN '> $1M'
          ELSE 'Other/No Data' 
        END AS customer_annual_revenue_group_ob*/
    , CASE WHEN F.customer_annual_revenue_ob>0 and F.customer_annual_revenue_ob<=500000 THEN '$0 - $500K'
           WHEN F.customer_annual_revenue_ob>500000 and F.customer_annual_revenue_ob<=1500000 THEN '$500K - $1.5M'
           WHEN F.customer_annual_revenue_ob>1500000 THEN '> $1.5M'
--           WHEN customer_annual_revenue>1000000 THEN '> $1M'
           ELSE 'Other/No Data'
        END AS customer_annual_revenue_group_ob
   ...
   , CD.has_calc_revenue
   --USAGE SPLIT
   , case when USG.avg_util_l3m >= 0.7 then 'High Usage'
            when USG.avg_util_l3m >= 0.3 then 'Medium Usage'
            when USG.max_util_l6m >= 0.6 then 'Sporadic'
            else 'Low MU'
            end as exp_usage_bkt
   ...
   , CASE WHEN cd.is_approved = 1 THEN f.ob_bucket_retro
        ...
        ELSE f.ob_bucket_actual -- New logic for ob bucket score for not approved customers
        END ob_risk_bucket
   ...
   , CASE WHEN FD.tenure_status = 'Tenure <60' THEN F.ob_bucket_retro
          WHEN FD.tenure_status = 'Tenure >=60' THEN FD.og_bucket
  ...
  -- , AUW_STATUS__C
   ...
   , CASE WHEN YEAR(AUW_PRE_DOC_REVIEW_START_TIME_WEEKEND)>=2025 
					THEN COALESCE(APPROVED_UW_CREDIT_LIMIT__C, AUW_PRE_DOC_APPROVED_LIMIT__C)
					WHEN AUW_PRE_DOC_REVIEW_START_TIME_WEEKEND is null then COALESCE(APPROVED_UW_CREDIT_LIMIT__C, AUW_PRE_DOC_APPROVED_LIMIT__C)
...
------------------------------------------------------- CUSTOMERS DATA -------------------------------------------------------
    ...
    (SELECT fbbid
         ...
         ,CASE WHEN first_account_size_accounting_software = 0 THEN NULL ELSE first_account_size_accounting_software END AS first_account_size_accounting_software_INT
	     ,CASE WHEN first_account_size_fi = 0 THEN NULL ELSE first_account_size_fi END AS first_account_size_fi_INT
	     , COALESCE(first_account_size_accounting_software_INT, first_account_size_fi_INT) * 12 calc_revenue ---- This only gives initial revenue; also, if both are null it will  be null, and nulls will be categorised under 'Other/No Data'
    ...
    LEFT JOIN FINAL_DACD_METRICS DACD -- MODIFIED: Using the new CTE that includes rolling sums
    ON CD.fbbid = DACD.fbbid

-------------------------------------------------------USAGE UTIL DACD(Can't think of taking os from fmd using dacd for this)--------------------------------

LEFT JOIN CTE_USAGE USG
ON DACD.FBBID = USG.FBBID
AND DACD.WEEK_END_DATE = USG.WEEK_END_DATE
...
LEFT JOIN 
    (SELECT DISTINCT fbbid
        ...
        , termunits
        , sub_product
        , customer_annual_revenue as customer_annual_revenue_ob -- i forgot how to calc annual rev
...
LEFT JOIN 
    (SELECT DISTINCT fbbid
         ...
         , CASE WHEN new_cust_filter='Existing Customer' THEN 'Tenure >=60' 
            ...
            END tenure_status
        /*, CASE 
            WHEN new_cust_filter = 'New Customer' THEN customer_annual_revenue
            ELSE 0
          END AS customer_annual_revenue_ob*/
        , customer_annual_revenue
        , customer_annual_revenue_group
        , og_bucket_retro 
        , og_bucket
        , og_bucket_group as og_bucket_group
        ,TENURE_BUCKET
    from indus.public.indus_key_metrics_filters_v2
    where edate=week_end_date
) FD
...
LEFT JOIN indus.public.alternative_lenders_vol_debt DL
ON DL.FBBID = DACD.fbbid
AND DL.trans_month = DATE_TRUNC('MONTH',DACD.WEEK_END_DATE)
...
LEFT JOIN (
    SELECT 
        TRY_CAST(fundbox_id__c AS NUMBER(38,0)) AS fbbid,
        date_trunc('week', createddate::date + 4)::date + 2 AS call_weekend_date,
        COUNT(fbbid) as call_count
    FROM external_data_sources.salesforce_nova.task 
    WHERE calltype = 'Inbound' 
      AND createddate >= '2024-01-01'
      AND TRY_CAST(fundbox_id__c AS NUMBER) IS NOT NULL
      group by 1,2
) in_call
  ON DACD.FBBID = in_call.fbbid
 AND DACD.week_end_date = in_call.call_weekend_date
...
LEFT JOIN CTE_FMD FMD
ON DACD.fbbid=FMD.fbbid
AND DACD.week_end_date=FMD.week_end_date

------------------------------------------------------WALLET SHARE---------------------------------------------------------
----- NEED TO FIGURE
/*LEFT JOIN CTE_FMD ORI
ON DACD.FBBID = ORI.fbbid
AND DATEDIFF('WEEK',ORI.LOAN_MONTH,DATE_TRUNC('MONTH',DACD.WEEK_END_DATE)) = 1*/
